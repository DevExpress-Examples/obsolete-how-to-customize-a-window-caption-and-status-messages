...
function ASPxGridListEditor(id, caption) {
		...
		try {
			var clientGridView = this.GetClientGridView(rowIndex);
			            ...
			            this.LogOperationError('The grid editing row is undefined');
			                       ...
			                       if(this.IsCommandAvailable(clientGridView, rowIndex, 'Edit')) {
			                  clientGridView.StartEditRow(rowIndex);
			                       }
			                       else {
	...
	this.InlineCancel = function(param, rowIndex, columnIndex) {
		...
		try {
			var clientGridView = this.GetClientGridView(rowIndex);
			               if(clientGridView){
			                   if(this.IsCommandAvailable(clientGridView, rowIndex, 'Cancel')) {
	...
	this.IsCommandAvailable = function(clientGridView, rowIndex, commandName) {
		...
		try {
			if(rowIndex != -1) {
			    var dataRow = clientGridView.GetDataRow(rowIndex);
			}
	...
	this.GetColumnEditor = function(rowIndex, columnIndex) {
		...
		try {
			...
			if(testControl) {
			 ...
			 return this.GetColumnEditorCore(rowIndex, columnIndex - this.GetGroupColumnCount());
			}
		}
		catch(e) {					
			if(e.operationError) {
...
function JSASPxTestParametrizedActionControl(id, caption) {
